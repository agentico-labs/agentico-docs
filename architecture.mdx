...
sequenceDiagram
  ...
  participant Store as Supermemory
  ...
  Eve->>Store: profile(scope, q = user message)
  Store-->>Eve: static + dynamic + relevant → fenced JSON
...
| **Sandbox** | eve gives every session its own sandbox; `sandbox/sandbox.ts` tags it and applies the user's egress allow-list. |
...
agent/
  ...
  sandbox/
    sandbox.ts             per-user egress allow-list (deny-all + approved) + tenant marker
  lib/
    scope.ts               TenantScope
    ...
    supermemory.ts         Supermemory client + per-scope container tags
    memory-store.ts        MemoryStore (Supermemory-backed)
    profile-store.ts       ProfileStore (Supermemory-backed)
    tool-store.ts          ToolStore (Supermemory-backed)
    run-user-tool.ts       static runner: executes user tool code in the sandbox
```

<Note>
  All three stores persist in [Supermemory](https://supermemory.ai), scoped per `(tenant, user)` by container tag, behind a small interface so the backend stays swappable. Memory is agentic — injected each turn from Supermemory's consolidated profile. Set `SUPERMEMORY_API_KEY`; see [Deployment](/guides/deployment#supermemory).
</Note>
