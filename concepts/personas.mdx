...
The store is [Supermemory](https://supermemory.ai), keyed by `(tenant, user)`. The first session that supplies an inline profile **provisions** the user — it persists the profile — so later sessions can omit the parameter and inherit it:
